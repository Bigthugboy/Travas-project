...
import (
	...
	"github.com/travas-io/travas/service"
	"log"
	"net/http"
...
type controller struct {
  TravisService service.TravasService
}

func  New(travasservice service.TravasService) controller{
	return controller{
		TravisService: travasservice,
	}
}
//Welcome : This method render the welcome page of the flutter mobile application
func (tr *controller) Welcome() gin.HandlerFunc {
...
func (tr *controller) Register( ctx *gin.Context) gin.HandlerFunc {
	var tourist model.Tourist
	if err := ctx.ShouldBindJSON(&tourist); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"message": err.Error()})

	}
	err := tr.TravisService.ProcessRegister()
	if err != nil {
